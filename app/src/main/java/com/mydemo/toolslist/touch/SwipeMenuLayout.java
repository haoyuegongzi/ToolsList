...
public class SwipeMenuLayout extends ViewGroup {
    ...
    private View mContentView;//addSubscribe,存储contentView(第一个View)

    //上一次的xy
    private PointF mLastP = new PointF();
    //addSubscribe,仿QQ，侧滑菜单展开时，点击除侧滑菜单之外的区域，关闭侧滑菜单。
    //增加一个布尔值变量，dispatch函数里，每次down时，为true，move时判断，如果是滑动动作，设为false。
    //在Intercept函数的up时，判断这个变量，如果仍为true 说明是点击事件，则关闭菜单。 
    private boolean isUnMoved = true;

    //addSubscribe,判断手指起始落点，如果距离属于滑动了，就屏蔽一切点击事件。
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mHeight = 0;
        int contentWidth = 0;//addSubscribe,适配GridLayoutManager，将以第一个子Item(即ContentItem)的宽度为控件宽度
        int childCount = getChildCount();

        //addSubscribe，为了子View的高，可以matchParent(参考的FrameLayout 和LinearLayout的Horizontal)
        ...
        if (isSwipeEnable) {
            ...
            switch (ev.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    isUserSwiped = false;//addSubscribe,判断手指起始落点，如果距离属于滑动了，就屏蔽一切点击事件。
                    isUnMoved = true;//addSubscribe , 仿QQ，侧滑菜单展开时，点击内容区域，关闭侧滑菜单。
                    iosInterceptFlag = false;//addSubscribe，每次DOWN时，默认是不拦截的
                    ...
                    mFirstP.set(ev.getRawX(), ev.getRawY());//addSubscribe,判断手指起始落点，如果距离属于滑动了，就屏蔽一切点击事件。
                    ...
                    if (mViewCache != null) {
                        if (mViewCache != this) {
                            ...
                            iosInterceptFlag = isIos;//addSubscribe，IOS模式开启的话，且当前有侧滑菜单的View，且不是自己的，就该拦截事件咯。
                        }
                ...
                case MotionEvent.ACTION_MOVE:
                    //addSubscribe，IOS模式开启的话，且当前有侧滑菜单的View，且不是自己的，就该拦截事件咯。滑动也不该出现
                    if (iosInterceptFlag) {
                        break;
                    }
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (isLeftSwipe) {
                        if (getScrollX() > mScaleTouchSlop) {
                            //addSubscribe,解决一个智障问题~ 居然不给点击侧滑菜单 我跪着谢罪
                            //这里判断落点在内容区域屏蔽点击，内容区域外，允许传递事件继续向下的的。。。
                            if (ev.getX() < getWidth() - getScrollX()) {
                                //addSubscribe,仿QQ，侧滑菜单展开时，点击内容区域，关闭侧滑菜单。
                                if (isUnMoved) {
                                    smoothClose();
                                }
    ...
    private boolean isExpand;//代表当前是否是展开状态 addSubscribe

    public void smoothExpand() {
        //展开就加入ViewCache：
        mViewCache = SwipeMenuLayout.this;

        //addSubscribe 侧滑菜单展开，屏蔽content长按
